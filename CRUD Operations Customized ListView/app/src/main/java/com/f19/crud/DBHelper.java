...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public void  addStudent(StudentModel STUDENTModel){
        //writing in database from studentmodel
    ...
    public void deleteStudent(String id){
        SQLiteDatabase db=this.getWritableDatabase();
        db.delete(STUDENT_TABLE,STUDENT_NAME+"=?",new String[]{id});
        db.close();

    }
    public boolean update(String name,int roll,boolean t) {
        SQLiteDatabase db = this.getWritableDatabase();


        Cursor cursor = db.rawQuery("SELECT * FROM STUDENT_TABLE WHERE STUDENT_NAME=?",new String[]{name});
        ...
        if (count> 0) {
            ...
            cv.put(STUDENT_ENROLL, t);
            long result = db.update(STUDENT_TABLE, cv, "STUDENT_NAME =?", new String[]{name});
